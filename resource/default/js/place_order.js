...
ec.placeOrder={
	init:function(){
		...
		ec.placeOrder.currenctyAndCountry();
		//取消勾选多次请求
	...
	currenctyAndCountry:function(){
		$('.country_else_selected').click(function (){
			$('.country_selected').show();
		});
		$('.currency_else_selected').click(function (){
	...
	specialEffects:function(){
		...
		//defalut rewards-add by wty(2015-09-30)
		$('#rewardsApply').on('click',function(){
			var new_rewards = $("#rewardsKeyShow").val();
			$("#rewardsKey").val(new_rewards);
		})
	},
	closePop:function(){
		...
		var json = json.data;
			...
			if(shipping.status=="200"){
				...
				$.each(shoppingData,function(i,n){
					...
					if(n.track!= -1 && (i !=3)){
						...
						var expires_str = '';
						if(n.id==3) expires_str = 'hide';
						if (n.available=='0') {
							trak+='<dl class="potion-info gray9 '+expires_str+'">';
						}else{
							trak+='<dl class="potion-info '+expires_str+'">';
						}
						...
						shoppingStr+='<dl>';
								...
								// shoppingStr+=	'<dl>'+
									shoppingStr+='<dt class="remember">'+
											'<input type="radio" name="options" id="shippingid'+n.id+'" class="radio" '+selected+'  '+disabled+'>'+
											'<label for="shippingid'+n.id+'">'+n.title+'</label>'+
										'</dt>'+
										'<dd>'+n.day+'</dd>'+
										'<dd>'+n.price+'</dd>'+
									'</dl>'+	
			...
			$.each(cartSummary,function(i,n){
				...
				cartSummaryStr+='<tr pid="'+n.pid+'">'+
					...
					'<td class="t4">'+n.price;
				if(n.save_price_num > 0){
					cartSummaryStr += '<p class="save-price">'+n.save_price+'</p>';
				}
				cartSummaryStr+='</td>'+
				'</tr>';
			})
			...
			//默认或者返回用户使用rewards值-add by wty(2015-09-30
			$('#rewardsKeyShow').val(json.default_able_use_rewards);
			$('#monetary').text(json.rewards_currency);
	...
	ajaxEvent:function(obj){
			...
			if(id=='useAddress'){
				...
				if(place_order_type=='paypal_ec_nologin'){
					...
					if(place_order_type == 'paypal_ec_login') action +='/paypal'
				}
