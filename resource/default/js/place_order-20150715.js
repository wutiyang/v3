...
ec.placeOrder={
	...
	specialEffects:function(obj){
		...
		rememberCheckbox.on('click',function(){
			$(this).closest('.frm').find('.frm-check').toggleClass('hide');		
		})
		$('.remember').find('[type="text"]').blur(function(){
			var me=$(this),
				...
				if( me.attr('id')=='rewardsKey'){
					if(!/^([0-9]+)*$/i.test(text) || parseInt(text)<=0 ){
	...
	addressUtils:function($address){
		var $address1=$('#street_1'),
		    $address2=$('#street_2')
		if($address.attr('id')!='street_2'){
			ec.placeOrder.utilsNull($address);
		}else{
			if($.trim($address2.val()) == '' || $.trim($address2.val())<1)
			return false;	
		}		
		var addressStrLen = $.trim($address1.val()).length + $.trim($address2.val()).length;
	...
	provinceUtils:function(province){
		var country=$('#country');
		if($.trim(province.val()) == '' && $.trim(country.val()) == 'AU'){
	...
	validate:function(){
		...
		ec.placeOrder.addressUtils($address2);
		ec.placeOrder.countryUtils(false);
		ec.placeOrder.cityUtils();
		if($('#country').val() == 'AU'){
			ec.placeOrder.provinceUtils($('#region'));
		}else{
			ec.placeOrder.provinceUtils($('#province'));
		}
	...
	defaultEvent:function(){
		$('#default').on('click',function(){
				...
				if(me.is(":checked")){
					$defualtValue.attr('value','1');	
				}else{
					$defualtValue.attr('value','0');	
				}
			})
	},
	eventBlur:function(){
		...
		$('#street_1,#street_2').on('blur',function(){
			ec.placeOrder.addressUtils($(this));
		})
		$('#zipcode').on('blur',function(){
			ec.placeOrder.utilsNull($(this));
		})	
		$('#province,#region').on('blur',function(){
			ec.placeOrder.provinceUtils($(this));
		})
	...
	dataList:function(json){
		var json = json.data;
			...
			$.each(json.address,function(i,n){
				var eb_default='',
					eb_str='';
				if(n.defaults!="0" ){
					eb_default='class="on"';
				...
				addressStr+='<li index="'+i+'" '+eb_default+'>'+
					'<div class="ads-user"><i></i><strong>'+n.last_name+'</strong></div>'+
					'<div class="ads-info">'+n.address+'</div>'+
			...
			if(shipping.status=="200"){
				...
				$.each(shoppingData,function(i,n){
					var disabled= n.available=='0' ? 'disabled' : '',
						...
						trak='',
						selected=n.selected=='0' ? '' : 'checked';
						if(n.track!='-1'){
							var trackDisabled=n.track=="0" ? '' : 'checked';
							trak+='<dl class="potion-info">'+
										'<dt class="remember">'+
											'<input type="checkbox" '+trackDisabled+' id="checkbox'+i+'" class="check">'+
			...
			$insurance.attr('checked',json.insurance==1 ? 'checked' :'');
			$itemsFirst.attr('checked',json.itemsFirst==1 ? 'checked' :'');
				
				
			var $payment=$('#payment'),
				paymentId=$payment.find('li[class="remember rem-list on"]').attr('paymentid'),
			...
			$.each(payment.data,function(i,n){
				...
				paymentStr+='<li class="remember rem-list" paymentid="'+n.id+'">'+
					'<img class="pay-img" src="'+n.picName+'">'+
					'<input class="method" id="'+n.id+'" type="checkbox" '+checked+' disabled="disabled"><label for="'+n.id+'"></label>'+
				'</li>';
			})
			...
			$('#rewardsPrice').html(rewards.price);
			rewardsMsg.hide();
			rewardsRemove.hide();
			...
			if(coupon.status=="200"){
				...
				ec.utils.addError(couponKey,rewards.msg);
				couponCheckbox.show();
			}
	...
	listEvent:function(tbool){
		var eventlist='#address li[class!="add-ads"],#shipping input,#payment li,#changeCountrySave,#useAddress',
			eventlist2=tbool ?  eventlist+',#couponApply,#rewardsApply,#place' :eventlist;
		$(eventlist2).on('click',function(e){
			...
			if(id=='changeCountrySave'){
				countryId.html($('#selectCountry .selected span').html());
				currencyId.html($('#selectCurrency .selected span').html());
			}
			var parameter={
					"addressId":addressId,
			...
			var place=$('#place'),
				isbool=(id=='place');
				var action=isbool ?  "/place_order/processOrder" : "/place_order/ajaxFresh";	
			...
			if(id=='useAddress'){
				...
				action=	index=='indexAdd' ? form.attr('actionAdd') : form.attr('actionEdit');
				ec.utils.removeError();
				ec.placeOrder.validate();
				var length=Pop.find('.error').length;
					if(length>0) return false;
					parameter= mergeJsonObject(parameter,strToObj(decodeURIComponent($('#addressForm').serialize(),true)));
			}
