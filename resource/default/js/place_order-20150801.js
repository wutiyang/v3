...
ec.placeOrder={
	...
	listEvent:function(tbool){
		var eventlist='#address li[class!="add-ads"],#shipping li[class!="potion-info"] input,#payment .method-list li,#changeCountrySave,#useAddress',
			eventlist2=tbool ?  eventlist+',#couponApply,#rewardsApply,#place,#itemsFirst,#insurance' :eventlist;
		...
		$(eventlist2).on('click',function(e){
			...
			radioCheckedId = radioCheckedId.replace('shippingid','');// add by wty
			var parameter={
					"addressId":addressId,
					...
					success: function(json){
						...
						if(status=="200"){
							if(isbool){
								...
								json.data.bool==true ? place.removeClass('icon-view-dis') : place.addClass('icon-view-dis');
