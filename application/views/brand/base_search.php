<div class="primary-top clear">
    <div class="primary-introduction">
        ...
        if(isset($curUri)){
	        ...
	        $url = isset($category_info)?$category_info['brand_category_url']:$brand_info['brand_url'];
        }
        ...
        $no_range = $all_param;
        $current_url = genURL($url,false,$no_range);
        ?>
        <ul>
            <li class="intr-list <?php if($sort==1) echo 'on'?>">
                ...
                $all_param['sort'] = 1;
                ?>
                <a href="<?php echo genURL($url,false,$all_param)?>" rel="nofollow"><?php echo lang('most_popular');?></a>
            </li>
            <li class="intr-list <?php if($sort==2) echo 'on'?>">
                <?php
                $all_param['sort'] = 2;
                ?>
                <a href="<?php echo genURL($url,false,$all_param)?>" rel="nofollow"><?php echo lang('new_arrivals');?></a>
            </li>
            <!--
            上下箭头请在span标签上加class
                箭头向上：icon-up
            ...
            <li class="intr-list <?php if($sort==3 || $sort==4) echo 'prices'?>">
                ...
                ?>
                <a href="<?php echo genURL($url,false,$all_param)?>" rel="nofollow"><?php echo lang('price');?><span class="<?php echo $icon_class;?>"><?php echo $cion_str;?></span></a>
            </li>
            <li>
                ...
                <div class="intr-last">
                    <form  method="post" name="priceForm">
                        <span class="price info"><?php echo $new_currency;?><input class="price-text front" value="<?php $search_price_range = isset($all_param['search_price_range'])?$all_param['search_price_range']:',';$search_price_range = explode(',',$search_price_range);if(count($search_price_range) == 1)echo '';else echo $search_price_range[0];?>" type="text" name="price-front"></span>
                        <strong>to</strong>
                        <span class="price info"><?php echo $new_currency;?><input class="price-text back" value="<?php if(count($search_price_range) == 1)echo '';else echo $search_price_range[1];?>" type="text" name="price-back"></span>
