...
class discountrangemodel extends CI_Model {
    ...
    public function getRangeByDiscountIds($discount_info=array()){
        ...
        global $mem_expired_time;
        $mem_key = md5("each_buyer_product_range_did_".json_encode($discount_ids));
    ...
    public function getNewRangeByDiscountIds($discount_info=array()){
    	$result = array();
    	if(!is_array($discount_info) || empty($discount_info))return $result;
    	$discount_ids = array();
    	foreach($discount_info as $discount)
    	{
    	...
    	global $mem_expired_time;
    	$mem_key = md5("each_buyer_all_discounts_range_by_did_".json_encode($discount_ids));
    	$list = $this->memcache->get($mem_key);
		if($list === false){
    	...
    	foreach($list as $value){
    		...
    		//$exclude_product_id = $value['exclude_product_id'];
    		$result[$product_id][] = array_merge($value,$discounts[$value['discount_id']]);
    	}
    	//echo "<pre>vvvvvvvvvvv";print_r($result);die;
    	return $result;
    }
    ...
    public function getRangeByDiscountInfo($discount_info = array(),$param = array()){
        ...
        $discount_ids = array_keys($discounts);
        global $mem_expired_time;
        $mem_key = md5("each_buyer_all_discounts_range_by_dinfo_".json_encode($discount_ids));
        $list = $this->memcache->get($mem_key);
		if($list === false){
            $this->database->slave->from('eb_discount_range');
	...
	public function getRangeExistsWithArray($product_ids){
        ...
        global $mem_expired_time;
        $mem_key = md5("each_buyer_all_discounts_range_by_pid_".json_encode($product_ids));
        $list = $this->memcache->get($mem_key);
		if($list === false){
            $this->database->slave->from('eb_discount_range');
            $this->database->slave->where('discount_range_status',STATUS_ACTIVE);
		...
		//对每个促销缓存
		foreach ($list as $k=>$v){
			$promote_range_id = $v['product_id'];
			$mem_key = md5("each_buyer_product_discount_range_".$promote_range_id);
...
//	public function getRangeExistsWithArray_bak($product_ids){
//		$result = array();
//		if(empty($product_ids) || !is_array($product_ids)) return $result;
//
//		$this->database->slave->from('eb_promote_range');
...
//******************捆绑促销 *****************************
	...
	public function getPidFullcutDiscounts($productId,$discountIds,$categoryIds){
		$result = array();
		if(!is_numeric($productId) || !$productId || !is_array($discountIds)|| empty($discountIds) || !is_array($categoryIds) || empty($categoryIds)){
			return $result;
		}
		
		global $mem_expired_time;
		$mem_key = md5("each_buyer_fullcut_discounts_".implode('_', $discountIds).$productId.implode('_', $categoryIds));
...
// 			echo $this->database->slave->last_query();exit;
...
// 				}
// 			}
			$result = array_keys($discounts);
			$this->memcache->set($mem_key, $result,$mem_expired_time['fullcut_discounts']);
		}
