...
class goodsmodel extends CI_Model {
    ...
    public function getProductList($product_ids = array(),$status = 0,$page_size = 0,$language_code = 'us',$is_simple = false){
        if(empty($product_ids) || !is_array($product_ids)) return array();
        
        $mem_key = md5("each_buyer_category_products_".json_encode($product_ids).'_status_'.$status."_pagesize_".$page_size);
    ...
    public function showProductList($data,$language_code='us'){
        ...
        foreach($data as $k=>$v){
            ...
            //给商品多语言赋初值
            $result[$product_id]['product_description_name'] = $v['product_name'];
            $result[$product_id]['product_description_content'] = '';
        ...
        if(!empty($descriptions)){
            foreach($descriptions as $description){
                if(isset($description['product_id']) && isset($result[$description['product_id']])){
    ...
    private function productWithIconNocache($product_ids = array()){
        ...
        $mem_key = md5('eb_eachbuyer_product_icon_by_pids_'.json_encode($product_ids));
        global $mem_expired_time;
        ...
        if($data===false){
            ...
            $query = $this->database->slave->get();
            $data = $query->result_array();
            $this->memcache->set($mem_key, $data,$mem_expired_time['product_icon']);
        }
    ...
    private function productWithSloganNocache($product_ids){
        $data = array();
        if(!is_array($product_ids))return $data;
        $mem_key = md5('eb_eachbuyer_product_slogan_by_pids_'.json_encode($product_ids));
        global $mem_expired_time;
...
//            $sql .= "where pslogan.product_slogan_status=1 and pslogan.product_id in (".$product_ids.") and slogan.slogan_status=1";
            ...
            $slave->from('eb_product_slogan as pslogan');
            $slave->join('eb_slogan as slogan','slogan.slogan_id=pslogan.slogan_id','left');
            $slave->where('pslogan.product_slogan_status',STATUS_ACTIVE);
            $slave->where('slogan.slogan_status',STATUS_ACTIVE);
            $slave->where_in('pslogan.product_id',$product_ids);
            $data = $slave->get()->result_array();
		...
		if(is_array($product_id) && !empty($product_id)){
			...
			if($data===false){
				...
				if(!empty($data)){
					...
					$data['slogan_info'] = $product_slogan;
					
					//多语言
					$description_info = $this->productDescriptionInfo($product_id,$language_code);
					$data['product_description_name'] = $data['product_name'];
                    $data['product_description_content'] = '';
					if(!empty($description_info)){
						if( isset($description_info['product_description_name']) && trim($description_info['product_description_name']) ){
							$data['product_description_name'] = $description_info['product_description_name'];
						}
						$data['product_description_content'] = (isset($description_info['product_description_content']) && trim($description_info['product_description_content'])) ?$description_info['product_description_content']:'';
					}
			...
			//对缓存中数据时间，计算判断
			if(!empty($data) && isset($data['slogan_info'])){
				$data['slogan'] = array();
				if(!empty($data['slogan_info'])){
					foreach ($data['slogan_info'] as $k=>$v){
						if($v['slogan_status'] == 1 && strtotime($v['slogan_time_start']) < time() && strtotime($v['slogan_time_end']) > time()){
							$data['slogan'][] = $v;
						}
					}
				}
				unset($data['slogan_info']);
			}
		}
		...
		if(is_array($product_id) && !empty($product_id)){
			...
			if($data === false){
				...
				if(!empty($data)){
					...
					$description_info = $this->productDescriptionInfo($product_id,$language_code);
					if(!empty($description_info)){
						$data['product_description_name'] = (isset($description_info['product_description_name']) && trim($description_info['product_description_name']))?trim($description_info['product_description_name']):$data['product_name'];
						$data['product_description_content'] = (isset($description_info['product_description_content']) && trim($description_info['product_description_content']) )?trim($description_info['product_description_content']):'';
					}
	...
	public function batchProductDescriptionInfo($product_ids,$language_code = "us"){
        ...
        $mem_key = md5("each_buyer_product_desc_by_pids_" . json_encode($product_ids) . "_languagecode_" . $language_code);
        $result = $this->memcache->get($mem_key);
		if($result === false){
            $this->database->slave->from('eb_product_description_'.$language_code);
            $this->database->slave->where_in('product_id',$product_ids);
	...
	public function getAllInfoWithArray($product_array,$language_code = "us",$status = 1){
		...
		if(!empty($all_base_infos)){
			...
			foreach ($all_base_infos as $icon_key=>&$icon_val){
				$pid = $icon_val['product_id'];
				$icon_mem_key = md5("each_buyer_goods_icon_".$pid);
		...
		if(!empty($all_base_infos)){
			...
			foreach ($all_base_infos as $slogan_info_key=>&$slogan_info_val){
				...
				if(array_key_exists($slogan_news_id, $slogan_all_infos)){
					if(!empty($slogan_all_infos[$slogan_news_id])){
						$s_v = $slogan_all_infos[$slogan_news_id];
						if(!isset($slogan_news_id) || $s_v['slogan_time_start'] || $s_v['slogan_time_end']) continue;
						if($s_v['slogan_status']==1 && strtotime($s_v['slogan_time_start']) < time() && strtotime($s_v['slogan_time_end']) > time()){
							$slogan_info_val['slogan'] = $s_v;
						}
					}
		...
		if(!empty($all_base_infos)){
			...
			$language_all_infos = array_merge($language_no_cache_infos,$language_cache_infos);
			foreach ($all_base_infos as $language_all_key=>&$language_all_val){
				...
				if(array_key_exists($language_again_pid, $language_all_infos)){
					$l_name = trim($language_all_infos[$language_again_pid]['product_description_name']);
					$language_all_val['product_description_name'] = !empty($l_name)?$l_name:$language_all_val['product_name'];
					$language_all_val['product_description_content'] = $language_all_infos[$language_again_pid]['product_description_content'];
				}else{
					$language_all_val['product_description_name'] = $language_all_val['product_name'];
	...
	public function categoryProductList($category_array){
        ...
        global $mem_expired_time;
        $category_str = is_array($category_array)?implode('_',$category_array):$category_array;
        $mem_key = md5("each_buyer_view_cate_product_category_id_".$category_str);
        ...
        $this->memcache->set($mem_key, $result,$mem_expired_time['view_cate_product']);
    }
		return $result;
	}
	...
	public function recommendProductList($category_array = array(),$product_ids = array(),$pagesize=18){
		...
		$mem_key = md5($mem_key.$language_code);
		global $mem_expired_time;
...
//				}
//			}
            $result = $this->showProductList($result,$language_code);
			$this->memcache->set($mem_key, $result,$mem_expired_time['search_recommend_product']);
		}
		//对缓存中数据时间，计算判断
		foreach ($result as $key=>&$val){
			$val['slogan'] = array();
			if(isset($val['slogan_info']) && !empty($val['slogan_info'])){
				foreach ($val['slogan_info'] as $ckey=>&$cval){
					if($cval['slogan_status'] == 1 && strtotime($cval['slogan_time_start']) < time() && strtotime($cval['slogan_time_end']) > time()){
						$val['slogan'][] = $cval;
					}
				}
			}
			unset($result[$key]['slogan_info']);
		}
	...
	public function alsolikeProductWithPid($product_id,$language_code= 'us', $pagesize=3){
		...
		if(is_numeric($product_id)){
			$mem_key = md5("each_buyer_recommendation_statistics_product_".$product_id.'_'.$language_code.'_'.$pagesize);
		}elseif(is_array($product_id)){
			$mem_key = md5("each_buyer_recommendation_statistics_product_".json_encode($product_id).'_'.$language_code.'_'.$pagesize);
		}else{
			return $result;
		}
...
/**
     ...
     * @param array $exclude_product_ids 剔除的商品ID集合
     * @param string $language_code 语言编码
     * @return array
     */
    public function getProductListByWhere($where = array(),$status = 0,$exclude_product_ids = array()){
        $result = array();
        if(!is_array($where)) return $result;
        $mem_key = md5("each_buyer_brand_products_".json_encode($where).'_status_'.$status);
        global $mem_expired_time;

        if(!$data = $this->memcache->get($mem_key)) {
            $slave = $this->database->slave;
            $slave->from('eb_product');
            if (!empty($where)) {
                foreach ($where as $column => $item) {
                    if (is_array($item)) {
                        $slave->where_in($column, $item);
                    } elseif($item == null){
                        $slave->where($column, $item, false);
                    } else {
                        $slave->where($column, $item);
                    }
                }
            }
            if (is_numeric($status) && $status) {
                $slave->where('product_status', $status);
            }
            //去掉移除的
            if(is_array($exclude_product_ids) && !empty($exclude_product_ids)){
                $slave->where_in('product_id', $exclude_product_ids,false);
            }
            $result = $slave->get()->result_array();

            $this->memcache->set($mem_key, $data,$mem_expired_time['each_buyer_brand_products']);
        }
        return $result;
    }

    public function getSepecialShipProductIds($type = 1){
        $result = array(375066,375074);
        return $result;
    }
}
