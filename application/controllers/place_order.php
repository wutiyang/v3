...
class Place_order extends Dcontroller {
	...
	public function index(){
		...
		//默认填充可用rewards
		$default_able_use_rewards = round( ($subtotal - $this->discount_price) * REWARDS_DISCOUNT,2);
		if($default_able_use_rewards > $this->total_rewards){
			$default_able_use_rewards = $this->total_rewards;
		}
		$this->_view_data['default_able_use_rewards'] = $default_able_use_rewards;
		//***************rewords**************end
	...
	public function ajaxAddress($type = 'nomail'){
		...
		//判断国家，州，税号等信息
		if($country_is_required===true && trim($phone)){
	...
	public function ajaxFresh($type=null,$insert_id = 0){
		...
		$rewardsValue = round($this->input->post("rewards"), 2);
		...
		$return_data['default_able_use_rewards'] = $this->able_user_rewards;
		//****************rewards end****************
		...
		$return_data['rewards_currency'] = $this->_view_data['new_currency'];
		//pay url start********
		$bool = false;
	...
	public function processOrder(){
		...
		if($black_list_result && !empty($black_list_result)){
			$url = gensslURL('success') . '?success=0&payment=balcklist';
		}else{
			//支付跳转url返回
			global $payment_list;
			$is_adyen_pay = false;
	...
	public function paypal_ec(){
		...
		$this->_view_data['default_able_use_rewards'] = 0;//未登录默认为0
		//***************rewords**************end
	...
	public function ecnologined_processOrder(){
		...
		$black_list_result = $this->checkCustomerInBlacklist();
		if($black_list_result && !empty($black_list_result)){
			$url = gensslURL('success') . '?success=0&payment=blacklist';
		}else{
			//paypal ec(判断支付是否成功)
	...
	private function formatCartData($cart_data){
		...
		foreach ($cart_data as $key=>$val){
			...
			$result[$key]['price'] = $val['product_info']['product_currency'].number_format(round($val['view_new_product_sum'],2), 2, '.', ',');
				...
				case COUPON_TYPE_GIFT://赠品
					...
					if( ($coupon_effect_info[0]['coupon_effect_price']==0) || $total_price > $coupon_effect_info[0]['coupon_effect_price']){
						...
						if(!empty($gift_product_info)){
							...
							$gift_product_info['product_off'] = $gift_product_info['view_product_off'] = $gift_product_array['price'];//固定价格为0
	...
	private function _giftInfoWithSku($sku,$quantity = 1,$price = 0){
		...
		$cart_list['product_sku_info'] = $sku_info;
		$cart_list['product_id'] = $sku_info['product_id'];
		$this->load->model("goodsmodel","product");
	...
	private function _filterNormalShipping($country_code,$weight_info,$sensitive = false){
		...
		//对物流排序，默认
		$new_able = array();
		if(isset($result['able'])){
			$new_able = $this->shippingSort($result['able']);
		}
		$result['able'] = $new_able;
		
		return $result;
	}
	
	//对物流排序，默认
	private function shippingSort($able_shipping){
		$new_able = $news = array();
		if(empty($able_shipping)) return $news;
		foreach ($able_shipping as $key=>$val){
			$key = $val['country_shipping_rule_shipping_code'];
			$news[$key] = $val;
		}
		if(array_key_exists('airmail', $news)){
			$new_able[] =  $news['airmail'];
		}
		if(array_key_exists('register_airmail', $news)){
			$new_able[] =  $news['register_airmail'];
		}
		if(array_key_exists('standard', $news)){
			$new_able[] =  $news['standard'];
		}
		if(array_key_exists('register_standard', $news)){
			$new_able[] =  $news['register_standard'];
		}
		if(array_key_exists('express', $news)){
			$new_able[] =  $news['express'];
		}
		return $new_able;	
	}
	...
	private function productInfoWithCartinfo($cart_data){
		...
		foreach ($cart_data as $extend_key=>$extend_val){
			...
			if(!isset($extend_val['old_product_info'])){
				$this->sku_status = false;//商品下架
				continue;
			}else{
				//对应sku价格
		...
		foreach ($cart_data as $cart_key=>&$cart_val){
			...
			if(isset($cart_data[$cart_key]['old_product_info']))unset($cart_data[$cart_key]['old_product_info']);
		}
	...
	private function addressList($default_address_id = 0){
		...
		if($default_address_id==0 || !is_numeric($default_address_id)){
			...
			if(empty($user_info)) return $address;
	...
	private function _normalShippingFormat($able_shipping_options,$selected_shipping_id,$track = 0){
		...
		if($selected_shipping_id){
			foreach ($able_shipping_options['able'] as $disable_k=>$disable_v){
				if($disable_v['selected']==1){
		...
		//如果选中方式物流不可用，默认选中第一个可用选项
		if($selected_status===false && $selected_shipping_id){
			$able_selected_num = 0; //available
			foreach ($able_shipping_options as $k=>&$v){
				if($v['available']==1 && $able_selected_num==0){
					$v['selected'] = 1;
					$this->shippingCharges = $v['single_price'];//当前汇率下价格
					if(in_array($selected_shipping_id,array(4,5))){
						$v['track'] =1;
						$this->shippingCharges += $v['single_trackprice'];
					}
