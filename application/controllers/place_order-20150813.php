...
class Place_order extends Dcontroller {
	...
	public function ajaxFresh($type=null,$insert_id = 0){
		...
		$currency = $this->input->post("currency");//支付币种,单位 ex:$
		//汇率改变  currency
	...
	public function ecnologined_processOrder(){
		...
		//创建用户及地址,登录
		$user_id = $this->_createPaypalecCustomer($email,$address,$paypalEcPayerid);
	...
	private function _userCouponInfo($cart_list,$coupon){
		...
		if(!empty($base_coupon_info)){
			...
			if(!empty($in_cart_list)){
				...
				$result['status'] = 404;
				$result['msg'] = "Sorry,this coupon code can only be used on specific products.";
			}
	...
	private function getCart($type = null){
		...
		//echo "place_order<pre>";print_r($cart_data);die;
		/*foreach ($cart_data as $key=>&$val){
			$sku = $val['product_sku'];
	...
	private function productInfoWithCartinfo($cart_data,$need_extend_price = 1){
		...
		foreach ($all_product_infos as $extend_key=>$extend_val){
			...
			$extend_all_product_infos[$extend_pid] = $this->singleProductWithPrice($extend_val);//扩展价格
		...
		foreach ($cart_data as $last_key=>&$last_val){
			...
			//详情
			if(isset($extend_all_product_infos[$pid]))$last_val['product_info'] = $extend_all_product_infos[$pid];
			//sku信息
		...
		//echo "<pre>bbbsss";print_r($cart_data);die;
		return $cart_data;
	}
	...
	private function _get_shipping_option_data($address_country,$cart_data,$cart_weight_info,$track = 0,$selected_shipping_id = 0){
		if(!empty($address_country)){
			...
			if($shipping_options_list['exists_rule']==1){
				...
				//echo "<pre>bbbbbbbbssssssssssss";print_r($able_shipping_options);die;
