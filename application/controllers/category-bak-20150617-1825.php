...
class category extends Dcontroller {
	...
	public function index($categoryId = 0, $page = 1){
		...
		//echo $return_product_ids_num;die;
		//具体返回展示的商品详情
		...
		//echo "<pre>";print_r($show_product_lists);die;
		...
		foreach ($cache_search_result['attribute_data'] as $k_attr=>&$v_attr){
			if(isset($v_attr['group']) && count($v_attr['group'])){
				...
				foreach ($v_attr['group'] as $v_attr_key=>&$v_attr_val){
					...
					else $v_attr_val['selected'] = false;
					
					//链接:所有已选中的属性名称-属性组名称 1/属性名称-属性组名称2/..../page.html
					$attrs_names = "";
					if(isset($attrIdsArray[$k_attr][$v_attr_key])){//选中状态时的链接
						$attrs_array = array();
						
						foreach ($attrNarrowSearch['attrGroups'] as $attr_for_link_key=>$attr_for_link_val){
							foreach ($attr_for_link_val as $attr_val_for_link_key=>$attr_val_for_link_val){
								$attrid_for_link_name = $cache_search_result['attribute_data'][$attr_for_link_key]['attribute_name'];
								$attrval_for_link_name = $cache_search_result['attribute_data'][$attr_for_link_key]['group'][$attr_val_for_link_key]['attribute_value_name'];
								if($attr_val_for_link_key!=$v_attr_key){
									$attrs_array[] = $attrid_for_link_name."-".$attrval_for_link_name;
									$new_attr_copy = $attrNarrowSearch['parameter'];
									unset($new_attr_copy[$attr_for_link_key."_".$attr_val_for_link_key]);
									$attrs_link = implode(",", array_keys($new_attr_copy));
								}
							}
							
						}
						
						$attrs_names = implode("/", $attrs_array);
						
						$v_attr_val['link'] = $attrs_names."/?attr=".$attrs_link;
					}else{
						$attrs_link = implode(",", array_keys($attrNarrowSearch['parameter']));
						$v_attr_val['link'] = "/?attr=".$attrs_link;
					}
		...
		*/
		...
		//echo "<pre>aaa";print_r($cache_search_result['all_product_list']);
		//echo "<pre>bbb";print_r($show_product_lists);die;
		/***********************************************/
	...
	//获取对应促销活动等折扣价格
	private function productWithPrice($data){
		...
		foreach ($data as $k=>&$v){
			if(isset($v['product_id']) && $v['product_id'] && is_numeric($v['product_id'])){
				...
				$v['product_discount_price'] = isset($discount_infos["discount_price"])?$discount_infos["discount_price"]:$front_price;
