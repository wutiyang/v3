	...
	class Dcontroller extends CI_controller {
		...
		public function productWithPrice($data){
			...
			foreach ($data as $k=>&$v){
				if(isset($v['product_id']) && $v['product_id'] && is_numeric($v['product_id'])){
					...
					//开始结束时间
					$v['discount_start'] = isset($discount_infos["discount_info"]['promote_discount_time_start'])?$discount_infos["discount_info"]['promote_discount_time_start']:'';
					$v['discount_end'] = isset($discount_infos["discount_info"]['promote_discount_time_end'])?$discount_infos["discount_info"]['promote_discount_time_end']:'';
		...
		protected function singleProductWithPrice($data,$sku_array = array()){
			if(isset($data['product_id']) && $data['product_id'] && is_numeric($data['product_id'])){
				...
				//开始结束时间
				$data['discount_start'] = isset($discount_infos["discount_info"]['promote_discount_time_start'])?$discount_infos["discount_info"]['promote_discount_time_start']:'';
				$data['discount_end'] = isset($discount_infos["discount_info"]['promote_discount_time_end'])?$discount_infos["discount_info"]['promote_discount_time_end']:'';
			}
		...
		public function singleProductDiscount($product_id,$market_price,$flush_cache = false){
            ...
            if($flush_cache){
                //该商品是否参与促销活动
                $discunt_range_list = $this->discountrange->getRangeExistsWithId($product_id,$flush_cache);
                echo "<pre>bbbbbssssssseeeeeeeee";print_r($discunt_range_list);die;
                if($discunt_range_list && !empty($discunt_range_list)){
                    ...
                    //所有discount_id
                    foreach ($discunt_range_list as $k=>$v){
                        array_push($discount_ids_array, $v['promote_discount_id']);
                    }

                    //所有折扣discount_id详情
                    $all_discount_infos = $this->discount->getBatchDiscountWithIds($discount_ids_array);
                    //找出符合条件，最大折扣
                    foreach ($all_discount_infos as $discount_k=>$discount_v){
                        ...
                        if($discount_effect_value > $max_discount_num && $start_time<time() && $end_time > time()){
                ...
                if(isset($active_range[$product_id])){
                    ...
                    foreach ($active_range[$product_id] as $discount_k=>$discount_v){
                        $start_time = isset($discount_v['promote_discount_time_start'])?strtotime($discount_v['promote_discount_time_start']):$defaultTime;
                        $end_time = isset($discount_v['promote_discount_time_end'])?strtotime($discount_v['promote_discount_time_end']):$defaultTime;
                        $discount_effect_value = isset($discount_v['promote_discount_effect_value'])?$discount_v['promote_discount_effect_value']:0;
                        $discount_id = isset($discount_v['promote_discount_id'])?$discount_v['promote_discount_id']:0;
                    ...
                    if($max_discount_num!=0){
                        ...
                        $discount_price = round($discount_price,2);
                        $result['discount_price'] = $discount_price;
                        //print_r('<br/>id:'.$product_id.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;discount_price:'.$discount_price.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;off:'.$max_discount_num);
                    }
                }
            }
		...
		public function singleProductDiscount_bak($product_id,$market_price,$flush_cache = false){
			$result = array();
			$discount_price = $market_price;//product_price_market，大价格
			$this->load->model("discountmodel","discount");
			$this->load->model("discountrangemodel","discountrange");
			...
			if($discunt_range_list && !empty($discunt_range_list)){
				...
				foreach ($discunt_range_list as $k=>$info){
					$discount_id = isset($info['promote_discount_id'])?$info['promote_discount_id']:0;
					//所有折扣数值-数组
					$discount_ids_array[$discount_id] =(int) $this->discount->getProductDiscount($discount_id,$flush_cache);
					//折扣详情
				...
				//***************************************************
				/*$discount_id = $discunt_range_list[0]['promote_discount_id'];
				 //商品折扣处理算法
				$discount_number = $this->discount->getProductDiscount($discount_id);
				*/
				$result['discount_number'] = $discount_number;
			
				if($discount_number!=0){
					$real_discount = (100-$discount_number)/100;
					$discount_price = $market_price*$real_discount;
					$discount_price = round($discount_price,2);
					$result['discount_price'] = $discount_price;
				}
		...
		public function singleProductBatchDiscount($all_discount_ids){
			...
			//echo "<pre>tttyyyy";print_r($all_discount_infos);die;
            $nowTime = requestTime();
			//找出符合条件，最大折扣
			foreach ($all_discount_infos as $discount_k=>$discount_v){
				//echo "<pre>bbbssss";print_r($discount_v);die;
				/*$start_time = strtotime($discount_v[0]['promote_discount_time_start']);
				$end_time = strtotime($discount_v[0]['promote_discount_time_end']);
				$discount_effect_value = $discount_v[0]['promote_discount_effect_value'];
				$discount_id = $discount_v[0]['promote_discount_id'];*/
        ...
        public function formatFullRangeGroupInfo($full_range = array()){
            ...
            if($sum >= $full_range['discount_condition']){
                ...
                $full_range['short_sum'] = $full_range['discount_condition'] - $sum;
                $full_range['view_short_sum'] = $full_range['view_discount_condition'] - $view_sum;
            }
