...
class Place_order extends Dcontroller {
	...
	public function index(){
		...
		//echo "<pre>fdfdfdfdf";print_r($shipping_options_data);die;
		...
		$shippingCharges = $insurancePrice = $rewardsBalance = $couponSavings = 0;
		$payPrice = $subtotal;
		$bool = null;
	...
	public function ajaxAddress(){
		...
		if($country_is_required==true){
			...
			if($address_id && is_numeric($address_id)){//更新
				...
				if($default == 1 && $address_id != $default_address[0]['address_id']){
					//取消原默认地址
	...
	public function ajaxAddress_nologin(){
		...
		if($country_is_required==true){
			//返回前端的地址信息（改变选中地址状态）
			$new_address_data[0]["first_name"] = $firstName;
			$new_address_data[0]["last_name"] = $lastName;
			$new_address_data[0]["address"] = $address;
			$new_address_data[0]["address2"] = "";
			$new_address_data[0]["city"] = $city;
			$new_address_data[0]["country"] = $country;
			$new_address_data[0]['country_name'] = $this->getCountryName($country);
			$new_address_data[0]["zipcode"] = $zipCode;
			$new_address_data[0]["mobile"] = $phone;
			$new_address_data[0]["cpf"] = $cpfcnpj;
			$new_address_data[0]["region"] = $province;
			$new_address_data[0]["defaults"] = 1;
			$new_address_data[0]['checked'] = 1;
	...
	public function ajaxFresh($type=null){
		...
		//*****汇率********start(重新获取汇率等信息)
		/*$currency_code = currentCurrency();//ex:'USD'
		$currency = "$";
		if($this->currency_format){
		...
		//echo "<pre>";print_r($new_address_data);
		...
		//echo "fdfdfdfdfdfdfd<pre>";print_r($shipping_options_data);die;
		...
		if($this->cart_noempty){
			$rewards_data = $this->_getRewardsData($user_id,$subtotal,$rewardsValue);
		...
		//最终支付费用 start******* 		
		$payPrice = $subtotal+$shippingCharges+$insurancePrice-$this->rewardsBalance-$this->couponSavings;
		$base_payPrice = round($subtotal/($this->currency_format['currency_rate']),2)+round($shippingCharges/($this->currency_format['currency_rate']),2);
		...
		$return_data["shippingCharges"] = $this->current_currency.' '.$shippingCharges;
		...
		if($type=="place_order"){
			...
			$return_data['pay_country'] = $this->current_currency;//支付国家
	...
	public function ecnologined_ajaxFresh($type = null){
		...
		$this->able_use_address = true;
		$address_data = $this->session->get('paypal_ec_address');
		$this->address_warehouse = $address_data[0]['country'];
		...
		//shipping option  start***************
		$shipping_options_data = $this->_get_shipping_option_data($address_data[0]['country'],$cart_data,$cart_weight_info,$shippingtrack,$shippingid);
		$return_data['shipping'] = $shipping_options_data;
		//shipping option  end***************
	...
	public function eclogined_ajaxFresh($type=null){
		...
		$return_data["shippingCharges"] = $currency.' '.$shippingCharges;
	...
	protected function _fetchAddressInfoFromPaypalECResponse($response) {
		...
		$new_address_data[0]["first_name"] = $address['address_firstname'];
		$new_address_data[0]["last_name"] = $address['address_lastname'];
		$new_address_data[0]["address"] = $address['address_address'];
		$new_address_data[0]["address2"] = "";
		$new_address_data[0]["city"] = $address['address_city'];
		$new_address_data[0]["country"] = $address['address_country'];
		$new_address_data[0]['country_name'] = $this->getCountryName($address['address_country']);
		$new_address_data[0]["zipcode"] = $address['address_zipcode'];
		$new_address_data[0]["mobile"] = '';
		$new_address_data[0]["cpf"] = '';
		$new_address_data[0]["region"] = $address['address_province'];
		$new_address_data[0]["defaults"] = 1;
		$new_address_data[0]['checked'] = 1;
	...
	private function _generateOrder($userId, $email, $result, $address, $payId = false, $payName = false){
		...
		foreach ($cart as $cart_k=>$cart_v){
			...
			//echo "<pre>dfdfdfdfdfd";print_r($cart_v);//die;
		...
		//echo $order_baseprice_subtotal."========".$result['able_user_baserewards'];die;
	...
	private function  _ruleshipping($address_warehouse,$cart_weight_info){
		...
		if(!empty($able_shipping_list['able'])){
			//计算快递价格
			...
			//echo "<pre>23456";print_r($rules);die;
			//不可用物流方式
			foreach ($able_shipping_list['unable'] as $unable_shipp_key=>$unable_shipp_val){
				...
				$un_shipping_price = $this->_shippingPrice($cart_weight_info['price_weight'],$type,$address_warehouse);
				//组合返回给前端的数据 start
			...
			//echo "<pre>last";print_r($rules);print_r($unrules);die;
			//$rules = array_merge($rules,$unrules);
		}	
	...
	private function formatShippigData($shipping_name,$price,$warehouse){
		...
		$data = array(
			...
			'register_standard'=>array(
				...
				'tips'=>"",'track'=>1,"trackTitle"=>"Track my package",'available'=>0,//是否禁用
				'price'=>$currency." 0.00",'trackPrice'=>"+".$currency.$standard,
	...
	private function formatUnShippigData($shipping_name,$price,$warehouse){
		...
		$data = array(
				...
				'airmail'=>array(
						...
						'tips'=>"Not available for this address",'track'=>0,"trackTitle"=>"Track my package",'available'=>1,//是否禁用
						'price'=>$currency." 0.00",'trackPrice'=>"+".$currency.$airmail,
						//'single_price'=>0,'single_trackprice'=>$airmail,'base_price'=>0,'base_trackprice'=>$base_airmail,
						'single_price'=>0,'single_trackprice'=>$airmail,'base_price'=>0,'base_trackprice'=>0,'true_trackprice'=>0,
				),
				...
				'express'=>array(
						...
						'tips'=>"Not available for this address",'track'=>0,"trackTitle"=>"",'available'=>1,//是否禁用
						'price'=>$currency.$price,'trackPrice'=>"",
						'single_price'=>$price,'single_trackprice'=>0,'base_price'=>$base_price,'base_trackprice'=>0,'true_trackprice'=>0,
				),
				'register_airmail'=>array(
		...
		try
		{
			...
			if($eid && is_numeric($eid)){
				...
				$contentParam = array(
						...
						'SHIP_WAY' => $shipping_list[$order['shipping_id']],
						'PAY_WAY' => $payment_list[$order['payment_id']],
	...
	private function _getUserAddressList($selected_address_id = 0){
		...
		if(!empty($address_data)){
			...
			foreach($address_data  as $address_key=>$address_val){
				...
				$new_address_data[$address_id]["first_name"] = isset($address_val['address_firstname'])?addslashes($address_val['address_firstname']):'';
				$new_address_data[$address_id]["last_name"] = isset($address_val['address_lastname'])?addslashes($address_val['address_lastname']):'';
				$new_address_data[$address_id]["address"] = isset($address_val['address_address'])?addslashes($address_val['address_address']):'';
				$new_address_data[$address_id]["address2"] = "";
				$new_address_data[$address_id]["city"] = isset($address_val['address_city'])?addslashes($address_val['address_city']):'';
				$new_address_data[$address_id]["country"] = isset($address_val['address_country'])?$address_val['address_country']:'';
				$new_address_data[$address_id]['country_name'] = isset($address_val['address_country'])?$this->getCountryName($address_val['address_country']):'';
				$new_address_data[$address_id]["zipcode"] = isset($address_val['address_zipcode'])?addslashes($address_val['address_zipcode']):'';
				$new_address_data[$address_id]["mobile"] = isset($address_val['address_phone'])?$address_val['address_phone']:'';
				$new_address_data[$address_id]["cpf"] = isset($address_val['address_cpfcnpj'])?$address_val['address_cpfcnpj']:'';
				$new_address_data[$address_id]["region"] = isset($address_val['address_province'])?$address_val['address_province']:'';
				$new_address_data[$address_id]["defaults"] = isset($address_val['address_default'])?$address_val['address_default']:0;
				if($selected_address_id==0){
					...
					if($address_id==$selected_address_id){
						$new_address_data[$address_id]["checked"] = 1;
						$this->address_warehouse = $address_val['address_country'];
					}else{
						$new_address_data[$address_id]["checked"] = 0;
					}
		...
		$first_address_data[$first_address_id] = $new_address_data[$first_address_id];
		unset($new_address_data[$first_address_id]);
		//$new_address_data = array_merge($first_address_data,$new_address_data);
		$new_address_data = $first_address_data+$new_address_data;
		return $new_address_data;
	}
			...
			if($shipping_options_list['exists_rule']==1){
				...
				if($able_shipping_options['able']){
					...
					foreach($able_shipping_options['able'] as $shipping_k=>&$shipping_v){
						if(($selected_shipping_id==1 || $selected_shipping_id==2)&& $track==1){
							$selected_shipping_id +=3;
						}
						if(($selected_shipping_id==4 || $selected_shipping_id==5)&& $track!=1){
							$selected_shipping_id -=3;
						}
						...
						if(in_array($shipping_v['id'], $clear_id)) unset($able_shipping_options['able'][$shipping_k]);
					}
					
					foreach($able_shipping_options['unable'] as $un_shipping_k=>&$unshipping_v){
						/*if($track==1){
							$un_clear_id = array(1,2);
						}else{
							$un_clear_id = array(4,5);
						}*/
						$un_clear_id = array(4,5);
						if(in_array($unshipping_v['id'], $un_clear_id)) unset($able_shipping_options['unable'][$un_shipping_k]);
					}
					$able_shipping_options = array_merge($able_shipping_options['able'],$able_shipping_options['unable']);
					$shipping_options_data = array(
							'msg'=>'OK',
							'status'=>200,
	...
	private function _getRewardsData($user_id,$subtotal,$rewardsValue = null){
		...
		if(!empty($user_rewards)){
			...
			if($total_rewards){
				$old_total_rewards = $total_rewards;
				if($this->currency_format) {
				...
				if($total_rewards > $product_rewards){
					...
					if($this->currency_format)$this->able_user_baserewards = round($subtotal/($this->currency_format['currency_rate']),2);
		...
		if($rewardsValue && is_numeric($rewardsValue)){
			if($rewardsValue > $this->able_user_rewards){
				...
				$this->base_rewardsbalance = $this->able_user_baserewards;
				$this->use_rewards_status = true;//可以使用rewards
			}
			$return_data['rewards']['status'] = $rewards_status;
			$return_data['rewards']['rewardsPrice'] =  $currency." ".$this->able_user_rewards;
	...
	public function _createPaypalecCustomer($email,$address){
		...
		if(!empty($exists_paypalec_user)){
			...
			//取消原默认地址
			$this->customer->editAddress($user_id, array('address_default'=>0));
		}else{
			//创建账号,初始密码为12345678
	...
	protected function _processPaypalECRequest($paypalEcToken, $paypalEcPayerid, $order) {
		//$this->load->library('Payment');//加载支付模块
		...
		if(isset($response['ACK']) && $response['ACK'] == 'Success') {
			...
			//$order['billing_address']['address2'] = id2name('address2','');
			$order['billing_address']['city'] = id2name('order_address_city',$order);
			$order['billing_address']['province'] = id2name('order_address_state',$order);
			$order['billing_address']['zipcode'] = id2name('order_address_postalcode',$order);
			$order['billing_address']['country'] = id2name('order_address_country',$order);
			$order['billing_address']['mobile'] = id2name('order_address_phone',$order);
			//$this->payment->markOrderPaid($order,id2name('TRANSACTIONID', $response), $order['email']);
			*/
			$this->load->model('ordermodel','ordermodel');
			$this->ordermodel->updateOrderWithCode($order['order_code'],array('order_status'=>OD_PAID,'order_time_pay'=>date('Y-m-d H:i:s' ,time())));
			...
			$params = array(
					...
					'MEMBER_NAME' => $order['order_address_firstname']." ".$order['order_address_lastname'],
