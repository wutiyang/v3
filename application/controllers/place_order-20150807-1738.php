...
class Place_order extends Dcontroller {
	...
	//页面上显示效果，最终处理(track,哪种物流的展示)--只处理普通的物流配送方式
	private function _normalShippingFormat($able_shipping_options,$selected_shipping_id = 0,$track = 0){
		$disable_id_inofs = $able_id_infos = array();
		$return_shipping_data = array();
		$first_able_shipping_id = 0;
		
		foreach ($able_shipping_options['able'] as $shipping_k=>&$shipping_v){
			$able_id_infos[$shipping_v['id']] = $shipping_v;
			if($first_able_shipping_id==0) $first_able_shipping_id = $shipping_v['id']; 
		}
		foreach ($able_shipping_options['unable'] as $unshipping_k=>&$unshipping_v){
			$disable_id_inofs[$unshipping_v['id']] = $unshipping_v;
		}
		
		//start**********提取物流方式*******
		//airmail
		if(array_key_exists(1, $able_id_infos)){
			$return_shipping_data[0] = $able_id_infos[1];
		}elseif(!array_key_exists(1, $able_id_infos) && array_key_exists(4, $able_id_infos)){
			//此时track是必须选中项
			$track_available = 1;//必选（固定的，必须选中）
			$able_id_infos[4]['$track_available'] = $track_available;
			$return_shipping_data[0] = $able_id_infos[4];
		}else{
			$return_shipping_data[0] = isset($disable_id_inofs[1])?$disable_id_inofs[1]:$disable_id_inofs[4];
		}
		//standard
		if(array_key_exists(2, $able_id_infos)){
			$return_shipping_data[1] = $able_id_infos[2];
		}elseif(!array_key_exists(2, $able_id_infos) && array_key_exists(5, $able_id_infos)){
			//此时track是必须选中项
			$track_available = 1;//必选（固定的，必须选中）
			$able_id_infos[5]['$track_available'] = $track_available;
			$return_shipping_data[1] = $able_id_infos[5];
		}else{
			$return_shipping_data[1] = isset($disable_id_inofs[2])?$disable_id_inofs[2]:$disable_id_inofs[5];
		}
		//express
		$return_shipping_data[2] = array_key_exists(3, $able_id_infos)?$able_id_infos[3]:$disable_id_inofs[3];
		//end**********提取物流方式*******
		
		//start********选中配送物流********
		if($selected_shipping_id==0 || !array_key_exists($selected_shipping_id, $able_id_infos)){//默认页情况，选中第一个可用物流，或选择的物流不在范围内时
			if(!empty($able_id_infos)){
				foreach ($return_shipping_data as $defalut_k=>&$defaule_v){
					if($first_able_shipping_id==$defaule_v['id']){
						$defaule_v['selected'] = 1;
						//运费
						$this->shippingCharges = $defaule_v['single_price'];//当前汇率下价格
						if(in_array($defaule_v['id'], array(4,5))) $this->shippingCharges +=  $defaule_v['single_trackprice'];
					}else $defaule_v['selected'] = 0;
				}
			...
			if(!empty($able_id_infos)){
				foreach ($return_shipping_data as $defalut_key=>&$defaule_val){
					if($selected_shipping_id==$defaule_val['id']){
						$defaule_val['selected'] = 1;
						//运费
						$this->shippingCharges = $defaule_val['single_price'];//当前汇率下价格
						if(in_array($defaule_val['id'], array(4,5))) $this->shippingCharges +=  $defaule_val['single_trackprice'];
					}
					$defaule_val['selected'] = 0;
				}
				//该物流可用
				$this->able_use_shipping = true;
			}	
		...
		echo $selected_shipping_id.'===='.$track.'====';
		echo "<pre>testtest";print_r($return_shipping_data);die;
		//end********选中配送物流********
		//echo "<pre>testtest";print_r($able_id_infos);print_r($disable_id_inofs);die;
		$return_data = array(
				'msg'=>'OK',
				'status'=>200,
				'data'=>$return_shipping_data,
		);
		$this->able_use_shipping = true;
		return $return_data;
	 ...
	 * @return array
	 */
	private function formatShippigData_bak($shipping_name,$price,$warehouse){
		$airmail = BASE_TRACK;
		$base_price = $price;
		$base_airmail = $airmail;
		$standard = (strtoupper($warehouse)=="DE")?8.69:2.19;
	...
	private function _normalShippingFormat_bak($able_shipping_options,$selected_shipping_id,$track = 0){
		$able_id_array = array();
		//根据track，排除不需要
