...
class Place_order extends Dcontroller {
	...
	public function ajaxFresh($type=null){
		...
		$shipping_options_data = $this->_get_shipping_option_data($selected_address_info['address_country'],$cart_data,$cart_weight_info,$shippingtrack);
		
		//echo "<pre>";print_r($shipping_options_data);die;
	...
	public function ecnologined_ajaxFresh($type = null){
		...
		//shipping option  start***************
		$shipping_options_data = $this->_get_shipping_option_data($address_data[0]['country'],$cart_data,$cart_weight_info,$shippingtrack);
		$return_data['shipping'] = $shipping_options_data;
		//shipping option  end***************
	 ...
	 */
	private function _get_shipping_option_data($address_country,$cart_data,$cart_weight_info,$track=0){
			...
			if($shipping_options_list['exists_rule']==1){
				...
				if($able_shipping_options){
					...
					foreach($able_shipping_options as $shipping_k=>$shipping_v){
						...
						if(in_array($shipping_v['id'], $clear_id)) unset($able_shipping_options[$shipping_k]);
					}
					$shipping_options_data = array(
							'msg'=>'OK',
							'status'=>200,
